...
def handler_send_save(ltype, source, parameters):
	...
	if GROUPCHATS.has_key(groupchat):
		...
		fromnick=source[2]+u' from '+source[1]+u' at '+date+u' (UTC) asked me to pass to you the following message:\n\n'
		if len(args)>=2:
			nick = args[0]
			body = ' '.join(args[1:])
			if nick == 'botadmin':
				reply(ltype, source, u'i will pass it')
...
register_command_handler(handler_send_save, 'pass', ['muc','all'], 10, 'Memorizes a message and passes to the indicated nick as soon as the user join in the conference.', 'pass <nick> <message>', ['pass guy hello! i need to talk with you!!!'])
