...
def handler_idle_answ(coze, res, type, source, param):
	...
	if res:
		if res.getType()=='error':
			reply(type,source,u'jabber server not found, or it falls down, or it forbids to look this information')
			return
...
def handler_userinfo_idle(type, source, parameters):
	if GROUPCHATS.has_key(source[1]):
		...
		if nick==source[2]:
			reply(type,source,u'what must I say? ;)')
			return
...
register_command_handler(handler_idle, 'uptime', ['info','muc','all'], 10, 'Show uptime of certain server.', 'uptime <server>', ['uptine jsmart.web.id'])
