...
def handler_weather_weathercode(type, source, parameters):
	if not parameters:
		reply(type, source, u'something you wrote not that...')
		return
	if len(parameters)<=2:
		reply(type, source, u'some nonsense...')
		return
	results = ''
...
register_command_handler(handler_weather_weather, 'weather', ['info','all'], 10, 'Looks a weather from NOAA', 'weather <4th_letters_code_city>', ['weather wiii'])
register_command_handler(handler_weather_weathercode, 'code', ['info','all'], 10, 'Shows the code of city for viewing of weather', 'code <city>', ['code jakarta'])
