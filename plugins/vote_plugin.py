...
def handler_vote_pollopinion_add(type, source, parameters):
	...
	if POLLINGS.has_key(source[1]):
		if POLLINGS[source[1]]['started']:
			...
			return		
		if POLLINGS[source[1]]['finished']:
			reply(type, source, u'does not applied, the vote was ended')
			return					
		if POLLINGS[source[1]]['creator']['jid']==jid or POLLINGS[source[1]]['options']['admedit']==1 and has_access(jid,20,source[1]):
		...
		if parameters:
			if POLLINGS[source[1]]['creator']['jid']==jid or POLLINGS[source[1]]['options']['admedit']==1 and has_access(jid,20,source[1]):
				...
				if parameters[0]=='closed':
					...
					if parameters[1]=='1':
						reply(type,source,u'account of user enabled')
						POLLINGS[source[1]]['options']['nicks']=True
					else:
						reply(type,source,u'account of user disabled')
...
def vote_results(gch):
	...
	for opinion in sorted(answ,lambda x,y: int(x[0]) - int(y[0]),reverse=True):
		...
		if len(opinion)==3:
			...
			poll_text += u'•\t'+str(cnt)+u' place '+str(opinion[0])+u' votes\n\tQuestion: '+opinion[1]+u'\n'
			allv+=opinion[0]
	poll_text += u'Всего '+str(allv)+u' votes'
	return poll_text

register_command_handler(handler_vote_polloptions, 'voting*', ['voting','muc','all'], 10, 'Manages options vote. All 4 options:\n1) closed - defines, whether the vote will be open (only in a general chat) or closed (only private)\n2) nicks - defines, wheter the bot record voting, for subsequent issuance, together with the results of the voting\n3) admedit -defines, whether the administration of the conference have the opportunity to edit the vote\n4) time - to determine the time (in seconds) the vote will take. 0 - manual stop', 'voting* <option> <state>', ['voting* nicks 1','voting* time 600'])
register_command_handler(handler_vote_stoppoll, 'voting-', ['voting','muc','all'], 11, 'Stop the vote, all data is saved to continue voting.', 'voting- <option> <state>', ['voting- nicks 1','voting- time 600'])
register_command_handler(handler_vote_pollstart, 'voting+', ['voting','muc','all'], 11, ' To submit your views on the current voting.', 'voting+ <view>', ['voting+ yes'])
register_command_handler(handler_vote_vote, 'opinion', ['voting','muc','all'], 10, ' To submit your views on the current voting.', 'opinion <view>', ['opinion yes'])
register_command_handler(handler_vote_pollopinions, 'view', ['voting','muc','all'], 11, 'Commended the ongoing results of the vote in private, do not wish to vote at this.', 'view', ['view'])
register_command_handler(handler_vote_newpoll, 'voting', ['voting','muc','all'], 11, 'Creates a new vote, or sent to a vote in the current chat, if given the views.', 'voting [question]', ['voting винды - сакс!', 'voting'])
register_command_handler(handler_vote_pollopinion_add, 'item+', ['voting','muc','all'], 11, 'Add an item (1!) to the current vote.', 'item+ <your_item>', ['item+ yes'])
register_command_handler(handler_vote_pollopinion_del, 'item-', ['voting','muc','all'], 11, 'Removes the item from the list. Item number needed.', 'item- <item_number>', ['item- 5'])
register_command_handler(handler_vote_endpoll, 'result', ['voting','muc','all'], 11, 'Voting is completed and shows the results.', 'result', ['result'])
